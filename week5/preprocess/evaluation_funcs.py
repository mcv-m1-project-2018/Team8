import numpy as np
from preprocess.bbox_iou import bbox_iou
...
def performance_evaluation_pixel(pixelTP, pixelFP, pixelFN, pixelTN):
    ...
    pixel_precision   =  float(pixelTP) / float(pixelTP+pixelFP) if(pixelTP+pixelFP) else 0
...
def performance_evaluation_window(TP, FN, FP):
    ...
    precision   = float(TP) / float(TP+FP)  if(TP+FP) else 0
