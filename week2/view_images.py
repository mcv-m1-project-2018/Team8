...
import numpy as np
from candidate_generation_pixel import morf_method1, boundingBoxFilter_method1, boundingBox_ccl, boundingBox_sw
...
bigger_kernel = np.ones((8, 8), np.uint8)
biggerer_kernel = np.ones((14, 14), np.uint8)
biggerest_kernel = np.ones((20, 20), np.uint8)
#big_kernel = np.ones((6,6), np.uint8)
...
for im_path in dirs:
    ...
    bb_list = boundingBox_ccl(imagen)
    imagen = boundingBoxFilter_method1(imagen, bb_list)
